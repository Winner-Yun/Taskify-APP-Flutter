...
class _SpecialReminderScreenState extends State<SpecialReminderScreen> {
  ...
  void _showTaskDialog(bool isDark, {TaskModel? taskToEdit}) {
    ...
    if (taskToEdit != null) {
      ...
      // Parse Existing Date (e.g. "Jan 20") back to numbers
      try {
        // Try parsing without year first (Standard Special Task format)
        ...
        // Fallback for older data with years
        try {
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        ...
                        if (titleError == null && dateErrorText == null) {
                          ...
                          // 1. Create a dummy date for formatting display string "Jan 20"
                          // Use a leap year like 2024 to allow Feb 29 input
                          DateTime displayDateObj = DateTime(2024, m, d);

                          // NOTE: We keep display date in English format for consistency or translate it
                          // For now, let's keep it standard English short format for the list view
